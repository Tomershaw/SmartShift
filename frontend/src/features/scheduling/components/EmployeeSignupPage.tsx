...
// CHANGED: משתמשים בלודר הגלובלי דרך ה-hook (לא מרנדרים קומפוננטה מקומית)
// עדיף - קריא ופשוט
...
export default function EmployeeSignupPage() {
  const { user } = useAuth();
  const { show, hide } = useLoading(); // CHANGED

  // שבוע הבא - ראשון עד חמישי
  const [weekStart] = useState<Date>(ilNextSunday); // lazy init
  const days = useMemo(
  ...
  // CHANGED: שליטה בלודר הגלובלי לפי dataReady
  useEffect(() => {
    if (!dataReady) {
      show("טוען משמרות והרשמות...");
    } else {
      hide();
    }
    return () => hide();
  }, [dataReady, show, hide]); // CHANGED

  // על שינוי שבוע - איפוס סטייט
  useEffect(() => {
    setLocalState({});
    setDayStatus(
    ...
    (async () => {
      try {
        ...
        for (const s of shifts) {
          ...
          const iso = ilISO(d);
          if (!isoToFirstShiftId[iso]) isoToFirstShiftId[iso] = s.id;
        }
        ...
        if (active) {
          ...
          setShiftsLoaded(true); // לא חוסם טעינת הרשמות
        }
      }
    })();
  ...
  // שלב 2: טעינת הרשמות אחרי משמרות
  useEffect(() => {
      ...
      try {
        ...
        for (const r of regs) {
          if (r.status === 0 || r.status === 1) {
            if (r.shiftDate) lockedDays.add(r.shiftDate);
          }
        }
        ...
        setLocalState(prev => {
          const copy = { ...prev };
          for (const iso of lockedDays) copy[iso] = null;
          return copy;
        });
  ...
  // CHANGED: אין return של לודר מקומי. העמוד תמיד מרונדר;
  // הלודר הגלובלי מכסה עד ש-dataReady=true.
    ...
    >
        ...
        <div className="rounded-2xl bg-gradient-to-r from-sky-500/10 via-teal-500/10 to-emerald-500/10 border border-slate-200 p-6">
          <div className="flex flex-col items-center text-center gap-2">
            ...
            <p className="text-slate-600">
              ...
              מוצגת לשבוע הבא בלבד. בחר משמרת אחת לכל יום בין ראשון לחמישי.
            </p>
      ...
      <section className="grid grid-cols-1 gap-5 sm:grid-cols-2 lg:grid-cols-5">
        {days.map(d => {
          ...
          // כפתורים פעילים רק כשהיום לא נעול ויש משמרת
          const buttonsEnabled =
            hasShiftId && (status === "idle" || status === "error");
      ...
      <div className="mt-8 flex items-center justify-end gap-2">
        ...
        <button
          ...
          className={`rounded-xl px-5 py-2 text-sm text-white ${
            submitting || draftRequests.length === 0
